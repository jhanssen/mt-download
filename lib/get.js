...
function prepare(opts, length)
{
    return new Promise((resolve, reject) => {
        if (typeof length == "number") {
            // preallocate file
            tmp.file({ postfix: ".mtdlm", dir: opts.path, keep: true }, (err, path, fd) => {
                if (err) {
                    reject(err);
                    return;
                }
            ...
            tmp.file({ postfix: ".mtdls", dir: opts.path, keep: true }, (err, path, fd) => {
                if (err) {
                    reject(err);
                    return;
                }
...
{
    return new Promise((resolve, reject) => {
        prepare(opts, length).then((path, fd) => {
            const threads = opts.threads || 2;
            resolve(new Controller(threads, path, fd, opts, length));
        }).catch(err => {
            reject(err);
        });
    });
...
{
    return new Promise((resolve, reject) => {
        prepare(opts).then((path, fd) => {
            resolve(new Controller(1, path, fd, opts, -1));
        }).catch(err => {
            reject(err);
        });
    });
